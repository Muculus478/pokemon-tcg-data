        ...
        if ($card.rarity -ne $null -and $includedRarities -contains $card.rarity) {
            ...
            if (-not $hasExcludedSubType) {
                ...
                foreach ($attack in $card.attacks) {
                    $attackDescription = "$($attack.name): Cost=[$($attack.cost -join ', ')], EnergyCost=[$($attack.convertedEnergyCost)], Damage=$($attack.damage), Text=$($attack.text)"
                    $attackDescriptions += $attackDescription
                }
                $abilityDescriptions =@()
                foreach ($ability in $card.abilities) {
                    $abilityDescription = "$($ability.name): Effect=[$($ability.text)]"
                    $abilityDescriptions += $abilityDescription
                }
                $attacksFormatted = $attackDescriptions -join " | "
                $abilitiesFormatted = $abilityDescriptions -join " | "
